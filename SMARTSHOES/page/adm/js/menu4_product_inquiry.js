...
function add_list(elem, data, name, copy_elem){
    ...
    if(name == "num"){
        ...
        var idx = {product_inquiry_idx: data.idx};
        elem.onclick = function(){
            gu.confirm({
                description : "제품문의 상세 내용을 확인하시겠습니까?", //내용(string 문자열) 필수
                title : null,  //제목(string 문자열) null이면 "알림"으로 처리함
                positive_method : "product_inquiry_detail", //예를 눌렀을경우 호출될 메소드function 이름(string 문자열) null일 경우 메소드를 실행하지않음
        ...
        elem.setAttribute("product_inquiry_idx", data["idx"]);
        elem.classList.add("select-custom");
        $(elem).trigger("change");
        elem.onchange = function(){
            request_product_inquiry_is_answer_change(this.getAttribute("product_inquiry_idx"),this.value);
        }
    }
}
...
function product_inquiry_detail(json) {
    json = JSON.parse(json);
    location.href = "?ctl=move&param=adm&param1=menu4_product_inquiry_detail&product_inquiry_idx=" + json.product_inquiry_idx;
}

function request_product_inquiry_is_answer_change(product_inquiry_idx, is_answer) {
    lb.ajax({
        type:"JsonAjaxPost",
        list : {
            ctl:"AdminMenu4",
            param1:"request_product_inquiry_is_answer_change",
            target : product_inquiry_idx,
            is_answer : is_answer
        },
        action : lb.obj.address, //웹일경우 ajax할 주소
        response_method : "response_product_inquiry_is_answer_change", //앱일경우 호출될 메소드
        havior : function(result){
            //웹일 경우 호출될 메소드
            // console.log(result);
            result = JSON.parse(result);
            response_product_inquiry_is_answer_change(result);
        }    
    });
}

function response_product_inquiry_is_answer_change(result){
    if(result.result == "1"){
        gu.alert({
