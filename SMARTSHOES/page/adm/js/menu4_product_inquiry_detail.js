...
function page_init() {
    lb.ajax({
        ...
        list: {
            ctl: "AdminMenu4",
            param1: "request_product_inquiry_detail",
            target: page_param.product_inquiry_idx,
        },
        action: "index.php",
        havior: function(result) {
            // console.log(result);
            result = JSON.parse(result);
            response_product_inquiry_detail(result);
        }
    })
}

function response_product_inquiry_detail(result) {
    // console.log(result);
    if(result["result"] == "1") {
        ...
        alert("잘못된 접근입니다.");
        location.href = "?ctl=move&param=adm&param1=menu4_product_inquiry";
    }
}
